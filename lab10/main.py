...
def insert_name(connection, name):
    try:
        with connection.cursor() as cursor:
            cursor.execute("INSERT INTO names (name) VALUES (%s) RETURNING name_id;", (name,))
...
def update_phone(connection, name_id, new_phone):
    try:
        with connection.cursor() as cursor:
...
def delete_name(connection, name):
    try:
        with connection.cursor() as cursor:
            cursor.execute("DELETE FROM phones WHERE fk_name_id = (SELECT name_id FROM names WHERE name = %s LIMIT 1);", (name,))
                ...
                if event.type == pg.QUIT:
                    ...
                    if save_button.collidepoint(event.pos):
                        ...
                        if name_input.strip() and phone_input.strip():
                            name_id = insert_name(connection, name_input)
                            if name_id is not None:
                                update_phone(connection, name_id, phone_input)
                                phone_input = ""
                                name_input = ""
